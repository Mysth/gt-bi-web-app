...
class CostRevenueReportForm(forms.Form):
    android = forms.BooleanField(label="Android ", required=False)
    ios = forms.BooleanField(label="iOS ", required=False)
    country = forms.ChoiceField(label="Country ", choices=COUNTRY_LIST)
    startDate = forms.DateField(widget=MyDateInput(), label='Start Date ')
    endDate = forms.DateField(widget=MyDateInput(), label='End Date ')
    applicationName = forms.ChoiceField(
        label="Application Name ", required=True, choices=GAME_LIST)
    reportType = forms.ChoiceField(
        label="Report Time Type ", required=True, choices=REPORT_TYPE)
